...
** doxygen
+ [[~/Document/Robotics/ROS/doxygen/ros/ros.pdf][ros]]
** ros
...
****
** [[http://wiki.ros.org/ros_comm?distro=jade][ros_common]]
...
** [[http://wiki.ros.org/robot_model?distro=jade][robot_model]]
...
[[~/Wally/Journal/Figures/scrot/30792qpM.png]]
...
** [[http://wiki.ros.org/geometry?distro=jade][geometry]]
*** [[http://wiki.ros.org/tf][tf]]                                     :tf:
**** Package Summary
...
frames in a tree structure buffered in time, and lets the user transform
points, vectors, etc between any two coordinate frames at any desired
point in time.


***** What does tf do? Why should I use tf?

You want to *see* what tf can do instead of just reading about it? Check
out the [[/tf/Tutorials/Introduction%20to%20tf][tf introduction demo]].

A robotic system typically has many 3D
[[/geometry/CoordinateFrameConventions][coordinate frames]] that change
over *time*, such as a world frame, base frame, gripper frame, head
frame, etc. tf keeps track of all these frames over time, and allows you
...
tf can operate in a *distributed system*. This means all the information
about the coordinate frames of a robot is available to all ROS
components on any computer in the system. There is *no central server*
of transform information.

For more information on the design see [[/tf/Design][/Design]]

***** Paper

There is a paper on tf presented at TePRA 2013
[[/Papers/TePRA2013_Foote][Papers/TePRA2013_Foote]]

***** Tutorials

We created a set of [[/tf/Tutorials][tutorials]] that walk you through
using tf, step by step. You can get started on the
[[/tf/Tutorials/Introduction%20to%20tf][introduction to tf]] tutorial.
For a complete list of all tf and tf-related tutorials check out the
[[/tf/Tutorials][tutorials]] page.

There are essentially two tasks that any user would use tf for,
listening for transforms and broadcasting transforms.

Anyone using tf will need to listen for transforms:

-  *Listening for transforms* - Receive and buffer all coordinate frames
   that are broadcasted in the system, and query for specific transforms
   between frames. Check out the writing a tf listener tutorial
   [[/tf/Tutorials/Writing%20a%20tf%20listener%20%28Python%29][(Python)]]
   [[/tf/Tutorials/Writing%20a%20tf%20listener%20%28C%2B%2B%29][(C++)]].

To extend the capabilities of a robot you will need to start
broadcasting transforms.

-  *Broadcasting transforms* - Send out the relative pose of coordinate
   frames to the rest of the system. A system can have many broadcasters
   that each provide information about a different part of the robot.
   Check out the writing a tf broadcaster tutorial
   [[/tf/Tutorials/Writing%20a%20tf%20broadcaster%20%28Python%29][(Python)]]
   [[/tf/Tutorials/Writing%20a%20tf%20broadcaster%20%28C%2B%2B%29][(C++)]].

Once you are finished with the basic tutorials, you can move on to learn
about tf and time. The tf and time tutorial
[[/tf/Tutorials/tf%20and%20Time%20%28Python%29][(Python)]]
[[/tf/Tutorials/tf%20and%20Time%20%28C%2B%2B%29][(C++)]] teaches the
basic principles of tf and time. The advanced tutorial about tf and time
[[/tf/Tutorials/Time%20travel%20with%20tf%20%28Python%29][(Python)]]
[[/tf/Tutorials/Time%20travel%20with%20tf%20%28C%2B%2B%29][(C++)]]
teaches the principles of time traveling with tf.

***** Code API Overview

#+BEGIN_HTML
  <div id="tf.2BAC8-Overview.content" dir="ltr" lang="en">
#+END_HTML

[[/tf/Overview][tf API Overview]]:

-  [[/tf/Overview/Data%20Types][Data Types]]
-  [[/tf/Overview/Transformations][Transformations and Frames]]
-  [[/tf/Overview/Broadcasting%20Transforms][Broadcasting Transforms]]
-  [[/tf/Overview/Using%20Published%20Transforms][Using Published
   Transforms]]
-  [[/tf/Overview/Exceptions][Exceptions]]
...
-  [[/geometry/CoordinateFrameConventions][Coordinate Frame
   Conventions]]

***** Command-line Tools

Although tf is mainly a code library meant to be used within ROS
[[/Nodes][nodes]], it comes with a large set of command-line tools that
assist in the debugging and creation of tf coordinate frames. These
tools include:

-  [[/tf#view_frames][view_frames]]: visualizes the full tree of
   coordinate transforms.
-  [[/tf#tf_monitor][tf_monitor]]: monitors transforms between frames.
-  [[/tf#tf_echo][tf_echo]]: prints specified transform to screen
-  [[/roswtf][roswtf]]: with the =tfwtf= plugin, helps you track down
   problems with tf.
-  [[/tf#static_transform_publisher][static_transform_publisher]] is a
   command line tool for sending static transforms.

You may also wish to use the [[/tf#tf_remap][tf_remap]] node, which is
a utility node for remapping coordinate transforms.

****** tf_monitor

=tf_monitor=

-  Print information about the current coordinate transform tree to
   console. For example:
...
-  Monitor a specific transform. For example, to monitor the transform
...
-  Print information about a particular transformation between a
   =source_frame= and a =target_frame=. For example, to echo the
...
-  Publish a static coordinate transform to tf using an x/y/z offset in
   meters and yaw/pitch/roll in radians. (yaw is rotation about Z, pitch
   is rotation about Y, and roll is rotation about X). The period, in
   milliseconds, specifies how often to send a transform. 100ms (10hz)
   is a good value.

=static_transform_publisher x y z qx qy qz qw frame_id child_frame_id  period_in_ms=

-  Publish a static coordinate transform to tf using an x/y/z offset in
   meters and quaternion. The period, in milliseconds, specifies how
   often to send a transform. 100ms (10hz) is a good value.

=static_transform_publisher= is designed both as a command-line tool for
manual use, as well as for use within [[/roslaunch][roslaunch]] files
for setting static transforms. For example:
...
=view_frames= is a graphical debugging tool that creates a PDF graph of
your current transform tree.

#+BEGIN_EXAMPLE
    $ rosrun tf view_frames
#+END_EXAMPLE

You probably want to view the graph when you are done, so a typical
usage on Ubuntu systems is:
...
NOTE: See also [[/rqt_tf_tree][rqt_tf_tree]] that allows dynamic
introspection of the frames.

****** roswtf plugin

=roswtf= tf comes with a plugin for [[/roswtf][roswtf]] that
automatically runs whenever you run =roswtf=. This plugin will analyze
your current tf configuration and attempt to find common problems. To
run, just invoke =roswtf= normally:

#+BEGIN_EXAMPLE
    $ roswtf
#+END_EXAMPLE

**** [[https://github.com/ros/geometry][github]]                  :github:
**** [[http://wiki.ros.org/tf/Tutorials][Tutorials]]            :tutorial:
***** [[http://wiki.ros.org/tf/Tutorials/Introduction%20to%20tf][Introduce to tf]]
***** C++
****** [[http://wiki.ros.org/tf/Tutorials/Writing%2520a%2520tf%2520broadcaster%2520%2528C%252B%252B%2529][Writing a tf broadcaster (C++)]]
****** [[http://wiki.ros.org/tf/Tutorials/Adding%20a%20frame%20%28C%2B%2B%29][Adding a frame (C++)]]
****** [[http://wiki.ros.org/tf/Tutorials/tf%20and%20Time%20%28C%2B%2B%29][Learning about tf and time (C++)]]
****** [[http://wiki.ros.org/tf/Tutorials/Time%20travel%20with%20tf%20%28C%2B%2B%29][Time travel with tf (C++)]]
***** python
****** [[http://wiki.ros.org/tf/Tutorials/Writing%20a%20tf%20broadcaster%20%28Python%29][Writing a tf broadcaster (Python)]]
****** [[http://wiki.ros.org/tf/Tutorials/Writing%20a%20tf%20listener%20%28Python%29][Writing a tf listener (Python)]]
****** [[http://wiki.ros.org/tf/Tutorials/Adding%20a%20frame%20%28Python%29][Adding a frame (Python)]]
****** [[http://wiki.ros.org/tf/Tutorials/tf%20and%20Time%20%28Python%29][Learning about tf and time (Python)]]
****** [[http://wiki.ros.org/tf/Tutorials/Time%20travel%20with%20tf%20%28Python%29][Time travel with tf (Python)]]
***** [[http://wiki.ros.org/tf/Tutorials/Debugging%20tf%20problems][Debugging tf problems]]
***** [[http://wiki.ros.org/tf/Tutorials/Using%20Stamped%20datatypes%20with%20tf%3A%3AMessageFilter][Using Stamped datatypes with tf::MessageFilter]]
***** Setting up your robot with tf
****** [[http://wiki.ros.org/navigation/Tutorials/RobotSetup/TF][Setting up your robot using tf]]
****** [[http://wiki.ros.org/robot_state_publisher/Tutorials/Using%20the%20robot%20state%20publisher%20on%20your%20own%20robot][Using the robot state publisher on your own robot]]
****** [[http://wiki.ros.org/urdf/Tutorials/Using%20urdf%20with%20robot_state_publisher][Using urdf with robot_state_publisher]]
**** [[http://wiki.ros.org/tf/Overview][tf API overview]]
***** [[http://wiki.ros.org/tf/Overview/Data%20Types][Data Types]]
***** [[http://wiki.ros.org/tf/Overview/Transformations][Transformations and Frames]]
***** [[http://wiki.ros.org/tf/Overview/Broadcasting%20Transforms][Broadcasting Transforms]]
***** [[http://wiki.ros.org/tf/Overview/Using%20Published%20Transforms][Using Published Transforms]]
***** [[http://wiki.ros.org/tf/Overview/Exceptions][Exceptions]]
**** tf
...
3. run the node
   ...
   + $ rosrun microstrain_3dmgx2_imu imunode _port:=/dev/ttyUSB0
...
**** [[http://wiki.ros.org/perception_pcl][perception_pcl]]                                                  :pcl:
***** [[http://wiki.ros.org/pcl_ros?distro=jade][pcl_ros]]
** mapping                                                         :mapping:
...
*** rospy_tutorials
** IDE                                                                 :ide:
*** [[http://wiki.ros.org/rosemacs][rosemacs]]                      :emacs:
**** rosemacs安装
